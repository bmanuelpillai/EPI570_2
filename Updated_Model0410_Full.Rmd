...
#zeta is proportion of asymptomatic that become symptomatic at each timestep, kappa is the proportion of susceptible exposed that become vaccinated exposed at each timestep
# MALES
dS.m.ue <- (1-(0.5 * omega.m * chi * mu * num)) - ((1/8) * s.m.ue.num)

dS.m.e <- ((1/8) * s.m.ue.num) - (lambda.m.s.a * s.m.e.num) + (delta.s.a * i.m.s.a.num) + (delta.s.s * i.m.s.s.num) - (kappa.m * s.m.e.num)
...
dV.m.e <- ((1/8) * v.m.ue.num) - (lambda.m.v.a * v.m.e.num) + (delta.v.a * i.m.v.a.num) + (delta.v.s * i.m.v.s.num) + (kappa.m * s.m.e.num)
...
dS.f.e <- ((1/8) * s.f.ue.num) - (lambda.f.s.a * s.f.e.num) + (delta.s.a * i.f.s.a.num) + (delta.s.s * i.f.s.s.num) - (kappa.f * s.f.e.num)
...
dV.f.e <- ((1/8) * v.f.ue.num) - (lambda.f.v.a * v.f.e.num) + (delta.v.a * i.f.v.a.num) + (delta.v.s * i.f.v.s.num) + (kappa.f * s.f.e.num)
