...
**PENDING THOUGHTS**
- add external FOI?
- changing birth rate to be for entire population
...
#4. Define Additional parameters #########################################################
  # recovery rate: 1/duration of infection, allowing for differential clearing rate by vax status & symptomatic status 
    ...
    # zeta is rate at which a sub-population of asymptomatic become symptomatic at each timestep 
...
# MALES : SUSCEPTIBLE
  ...
  dS.m.ue <- ((0.5 * (1-omega.m*chi) * nu*num)) +            # births
      ...
      # S populated by complement of proportion vaccinated*proportion immunogenic
  ...
  dS.m.e <- (age.in * uv.m.ue.num) + (-mu * uv.m.e.num) + (-age.out * uv.m.e.num) +  # general pop
            (-lambda.m.s * uv.m.e.num) +                                          # exposure
  ...
  dI.m.s.a <- (-age.out * uv.m.i.a.num) + (-mu * uv.m.i.a.num) +  # general pop
              (lambda.m.s * uv.m.e.num) +                    # infection
              (-zeta.s * uv.m.i.a.num) +                        # convert to symptomatic
              (-delta.s.a * uv.m.i.a.num)                     # recovery
    
  # infected symptomatic [uv.m.i.s.num]
  dI.m.s.s <- (zeta.s * uv.m.i.a.num) +                             # conversion to symptomatic
...
# MALES : VACCINATED
  ...
  dV.m.e <- (age.in * v.m.ue.num) + (-age.out * v.m.e.num) + (-mu * v.m.e.num) +    # gen pop
            (-lambda.m.v * v.m.e.num) +                                             # exposure
  ...
  dI.m.v.a <- (-age.out * v.m.i.a.num) + (-mu * v.m.i.a.num) +        # gen pop
              (lambda.m.v * v.m.e.num) +                            # infection
              (-zeta.v * v.m.i.a.num) +                               # convert to symp
...
# FEMALES : SUSCEPTIBLE
  ...
  dS.f.e <- (age.in * uv.f.ue.num) + (-mu * uv.f.e.num) + (-age.out * uv.f.e.num) +  # general pop
            (-lambda.f.s * uv.f.e.num) +                                          # exposure
  ...
  dI.f.s.a <- (-age.out * uv.f.i.a.num) + (-mu * uv.f.i.a.num) +  # general pop
              (lambda.f.s * uv.f.e.num) +                       # infection
              (-zeta.s * uv.f.i.a.num) +                        # convert to symptomatic
              (-delta.s.a * uv.f.i.a.num)                       # recovery
    
  # infected, symptomatic [uv.f.i.s.num]
  dI.f.s.s <- (zeta.s * uv.f.i.a.num) +                             # conversion to symptomatic
...
# FEMALES : VACCINATED
  ...
  dV.f.e <- (age.in * v.f.ue.num) + (-age.out * v.f.e.num) + (-mu * v.f.e.num) +    # gen pop
            (-lambda.f.v * v.f.e.num) +                                           # infection
  ...
  dI.f.v.a <- (-age.out * v.f.i.a.num) + (-mu * v.f.i.a.num) +        # gen pop
              (lambda.f.v * v.f.e.num) +                            # infection
              (-zeta.v * v.f.i.a.num) +                               # convert to symp
...
# (1.5)(16)
                    ...
                    nu = ((13/1000)/365), # this rate is the whole US, not age-group specific - needs fixing
                    mu.ue = (50/1e5)/365, mu = (100/1e5)/365, # these rates are estimated for teens & young adults, respecticaly
                    ...
                    # vaccine parameters
                    omega.m = seq(0.1,0.8, by = 0.2), omega.f = 0.65, 
                    chi = 0.97, kappa = (0.0025/365),
                    # symptomatic parameters
                    # (proportion symptomatic)*(rate of developing symptoms, 2-3 months), per day
                    zeta.v = ((0.05)*(3/12))/365 , zeta.s = ((0.1)*(3/12))/365
                    )
