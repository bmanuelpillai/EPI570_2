...
## INSTRUCTIONS ########
- each model has its own timed intervention (varying omega or kappa). both timed interventions start at year 15 (day 5475)
- each timed intervention runs through a series sensitivity analysis for the parameter(s) of interest
- both models use the same initial conditions, but have their own model/parameters/control objects
- currently all objects added to model & graphs depend on the model output being called the same thing (`mod`) - just for ease of running things
--> if you want to run a specific model, clear your environment, run the model fx and its specific parameters, then move down to the RUN chunks and run the control and model itself
...
```{r, parameters: time-varying kappa}
...
# RUN: TIMED OMEGA
```{r}
control.o <- control.dcm(nsteps = (100*365), new.mod = Vaccination.o)
...
# RUN: TIMED KAPPA
```{r}
control.k <- control.dcm(nsteps = (100*365), new.mod = Vaccination.k)
