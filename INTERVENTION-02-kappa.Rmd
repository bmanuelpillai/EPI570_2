...
catchup.males.percent<- model2 %>% 
  filter(time >= 29200) %>% 
  summarize(vac.male.percent = (v.m.ue.num + v.m.e.num + v.m.i.a.num + v.m.i.s.num)/ m.num ) 


catchup.females.percent<- model2 %>% 
  filter(time >= 29200)%>% 
  summarize(vac.female.percent = ( + v.f.e.num + v.f.i.a.num + v.f.i.s.num)/ f.num)

catchup.overall.percent <- model2 %>% 
  filter(time >= 29200) %>% 
...
legend("topright", legend = c("Vaccinated Male Prevalence", "Vaccinated Female Prevalence"), 
       col = c("blue", "pink"), pch = 19, cex = 0.5)
```
